...
pid_values=("pi-" "mu-")
# pid_values=("pi-")
# energy_values=(1 2.5 5 7.5 12.5 15 30)
...
if [ "$run_mode" == "test" ] ; then
    ...
    # Adjust the rapidity in runSimFlat
    sed -i $'51c\\\t\\\tkinematics->Kine(npart, PID, e-0.01, e+0.01, '"$rapidity_values"', '"$rapidity_high"', -pi, pi);' runSimFlat.C
    sed -i $''"297"'c\\'\t'push @DATAFILES, "$i '"$pid_values"' '"$energy_values"' 0 0 1";' MakeJob.pl
    if [ "$mode" == "simbfc" ]; then
        ./MakeJob.pl -d "output/simflat/eta_${rapidity_low_dir}/"$pid_values"e${energy_values}/Output" -o "output/test" -m "$mode" -u "$pid_values"e"$energy_values" -w -e -n 10000 -t
    fi
fi